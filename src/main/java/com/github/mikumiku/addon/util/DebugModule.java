...
//        }
    ...
    private String getPacketDetails(Packet<?> packet) {
        ...
        if (packet instanceof ClickSlotC2SPacket click) {
            int slot = click.slot(); // 被点击的槽位
            int button = click.button(); // 鼠标键
            SlotActionType actionType = click.actionType(); // 点击动作类型（PICKUP、QUICK_MOVE等）
            Int2ObjectMap<ItemStackHash> modifieded = click.modifiedStacks();// 被点击槽位上的物品
    ...
    private static String formatItemMap(Int2ObjectMap<ItemStackHash> map) {
        if (map == null || map.isEmpty()) return "无修改";

        StringBuilder sb = new StringBuilder();
        sb.append("修改槽位:\n");
        map.forEach((slot, stack) -> {
            ItemStackHash.Impl stackHash = (ItemStackHash.Impl) stack;
            String itemName = stackHash.item().getIdAsString();
            sb.append(String.format("  [%d] %s x%d\n", slot, itemName, stackHash.count()));
        });
        return sb.toString();
    }
