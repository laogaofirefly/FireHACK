...
public class BestPrinter extends BaseModule {
    ...
    private final Setting<Boolean> lavaHandling = slabSettings.add(
        new BoolSetting.Builder()
            .name("岩浆处理")
            .description("是否启用岩浆的特殊放置逻辑。需要岩浆桶。")
            .defaultValue(true)
            .build()
    );

    private final Setting<Boolean> waterHandling = slabSettings.add(
        new BoolSetting.Builder()
            .name("水源处理")
            .description("是否启用水源的特殊放置逻辑。需要水桶。")
            .defaultValue(true)
            .build()
    );
    ...
    private void placeBlockAtIndex(int index) {
        ...
        // 检查是否是岩浆方块，需要特殊处理
        if (lavaHandling.get() && targetState.getBlock() == Blocks.LAVA) {
            FluidState fluidState = targetState.getFluidState();
            boolean isFlowing = fluidState.getFluid() == Fluids.FLOWING_LAVA;
            placeLavaBlock(targetPos, isFlowing);
        }
        // 检查是否是水源方块，需要特殊处理
        else if (waterHandling.get() && targetState.getBlock() == Blocks.WATER) {
            placeWaterBlock(targetPos);
        }
        // 检查是否是半砖方块，需要特殊处理
        else if (slabHandling.get() && targetState.getBlock() instanceof SlabBlock) {
...
//            placeFacingBlock(targetPos, targetState, inventorySlot, Properties.HOPPER_FACING);
     ...
     * 放置后会消耗桶，玩家获得空桶
     */
    private void placeLavaBlock(BlockPos pos, boolean isFlowing) {

        // 岩浆桶需要使用 interactItem 而不是 interactBlock
        // 因为岩浆是通过使用物品来放置流体，而不是点击相邻方块

        // 使用 interactItem 直接使用岩浆桶
    ...
    private boolean isBlockDifferent(SchematicContext context) {
        boolean diffBlock = context.targetState.getBlock() != context.currentState.getBlock();
        if (diffBlock) {
            return true;
        }

        if (context.targetState.getBlock() == Blocks.LAVA || context.targetState.getBlock() == Blocks.WATER) {
            return context.targetState.getFluidState().getLevel() != context.currentState.getFluidState().getLevel();
        }
    ...
    private int findBlockInInventory(BlockState targetState) {
        ...
        if (targetBlock == Blocks.LAVA) {
            int slot = BagUtil.findItemInventorySlotGrim(Items.LAVA_BUCKET);
            if (slot != -1) {
                return slot;
            }
            // 如果没有岩浆桶，返回-1
        ...
        if (targetBlock == Blocks.WATER) {
            int slot = BagUtil.findItemInventorySlotGrim(Items.WATER_BUCKET);
            if (slot != -1) {
                return slot;
            }
            // 如果没有水桶，返回-1
