...
public class LiquidFiller extends BaseModule {
     ...
     * 失败位置缓存管理器，记录放置失败的位置
     */
    private final PositionCache failedCache = new PositionCache(2000L); // 2秒过期
    ...
    private final Setting<Boolean> bub = sgGeneral.add(new BoolSetting.Builder()
        .name("气泡柱不放")
        .description("防止意外填珍珠点")
        .defaultValue(true)
        .build()
    );
    ...
    public LiquidFiller() {
        super(CATEGORY_MIKU_BUILD, "自动填水填海", "自动将方块放置在您范围内的水源方块内。");
    }
    ...
    public void onActivate() {
        super.onActivate();
        timer = 0;
        failedCache.startCleanupThread();
    }

    @Override
    public void onDeactivate() {
        timer = 0;
        failedCache.shutdown(); // 关闭缓存管理器
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get() + 1), (int) Math.ceil(placeRange.get()), (blockPos, blockState) -> {
            ...
            // 跳过失败缓存
            if (failedCache.isInCache(blockPos)) {
                return; // 位置在缓存中，跳过
            }
            ...
            // 检查玩家是否可以在该位置放置方块
            if (!BaritoneUtil.canPlace(blockPos)) {
                return;
            }
            if (bub.get() && blockState.isOf(Blocks.BUBBLE_COLUMN)) {
                return;
            }
            // 添加方块
