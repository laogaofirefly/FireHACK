...
public class AutoLoginPlus extends BaseModule {
    ...
    private final SettingGroup sgImportExport = this.settings.createGroup("导入导出");
    ...
    // 导入输入框
    private final Setting<String> importInput = this.sgImportExport
        .add(
            new StringSetting.Builder()
                .name("导入账户")
                .description("粘贴账户列表，格式: 用户名:密码，每行一个")
                .defaultValue("")
                .build()
        );

    // 导出显示框
    private final Setting<String> exportOutput = this.sgImportExport
        .add(
            new StringSetting.Builder()
                .name("导出账户")
                .description("当前账户列表，格式: 用户名:密码，每行一个")
                .defaultValue("")
                .build()
        );

    // 导入按钮
    private final Setting<Boolean> importButton = this.sgImportExport
        .add(
            new BoolSetting.Builder()
                .name("执行导入")
                .description("点击导入上方输入框中的账户")
                .defaultValue(false)
                .visible(() -> false)
                .onChanged(this::onImportButtonClick)
                .build()
        );

    // 导出按钮
    private final Setting<Boolean> exportButton = this.sgImportExport
        .add(
            new BoolSetting.Builder()
                .name("执行导出")
                .description("点击将账户列表导出到上方输出框")
                .defaultValue(false)
                .visible(() -> false)
                .onChanged(this::onExportButtonClick)
                .build()
        );
     ...
     * 导入按钮点击回调
     */
    private void onImportButtonClick(Boolean value) {
        if (value) {
            performImport();
            importButton.set(false);
        }
    }
     ...
     * 导出按钮点击回调
     */
    private void onExportButtonClick(Boolean value) {
        if (value) {
            performExport();
            exportButton.set(false);
        }
    }
    ...
    private void performExport() {
        ...
        String exportText = accounts.entrySet().stream()
            .map(entry -> entry.getKey() + ":" + entry.getValue())
            .collect(Collectors.joining("\n"));

        exportOutput.set(exportText);

        if (debugMode.get()) {
            info("已导出 " + accounts.size() + " 个账户到输出框");
        }
    }
     ...
     * 同步内存中的账户到设置列表
     */
    private void syncAccountsToList() {
        List<String> list = accounts.entrySet().stream()
            .map(entry -> entry.getKey() + ":" + entry.getValue())
            .collect(Collectors.toList());
        accountList.set(list);
    }
