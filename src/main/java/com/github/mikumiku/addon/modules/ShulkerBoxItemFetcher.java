...
public class ShulkerBoxItemFetcher extends BaseModule {
    ...
    // Pickup tracking (参照 MassExtractor#tickPickupDrop 的追逐机制)
    private ItemEntity pickupDrop = null;       // 当前正在追逐的掉落物实体
    private boolean pickupPathed = false;       // 是否已发出寻路指令
    private int pickupSettleTicks = 0;          // 等待掉落物落地的 tick 计数
    private BlockPos pickupGoal = null;         // 上次寻路到的目标方块（用于检测漂移）
    private int pickupAttempts = 0;             // 总等待 tick 计数（超时上限）
    private int pickupBefore = 0;               // 打破前背包中潜影盒的数量
    private static final int PICKUP_SETTLE_TICKS = 20;  // 等掉落物落地最多 1 秒
    private static final int PICKUP_TIMEOUT_TICKS = 20 * 10; // 10 秒超时放弃
    ...
    private void breakShulkerBox() {
        ...
        Block block = mc.world.getBlockState(shulkerPos).getBlock();
        if (!(block instanceof ShulkerBoxBlock)) {
            if (logActions.get()) info("潜影盒已不存在（已被破坏）");
            changeState(State.PICKING_UP_SHULKER);
            baritone.getPathingBehavior().cancelEverything();
            return;
        }

        // 打破前快照背包中潜影盒数量（用于检测拾取成功）
        if (pickupBefore == 0) {
            pickupBefore = countShulkersInInventory();
            if (logActions.get()) dbg("打破前背包潜影盒数量: %d", pickupBefore);
        }
        ...
        // 不立即切换状态 —— 每 tick 持续调用直到 ShulkerBoxBlock 变为空气
    }

    private void pickUpShulkerBox() {
        int result = tickPickupDrop(shulkerPos);

        switch (result) {
            case 0 -> {
            } // BUSY — 继续追逐
            case 1 -> {  // DONE — 已拾取成功
                if (logActions.get()) info("潜影盒拾取成功");
                resetPickupState();
                changeState(State.FINISHED);
            }
            case 2 -> {  // GAVE_UP — 超时放弃
                warning("拾取潜影盒超时 — 放弃追逐");
                resetPickupState();
                changeState(State.FINISHED);
            }
        }
    ...
    /**
     * 单 tick 驱动拾取逻辑，参照 MassExtractor#tickPickupDrop。
     * 返回值：0=BUSY（仍在追逐），1=DONE（已拾取），2=GAVE_UP（超时放弃）。
     * 核心机制：定位掉落实体 → 等落地 → 追逐寻路 → 检测漂移重新寻路 → 数量增加即完成。
     */
    private int tickPickupDrop(BlockPos breakPos) {
        // 检测拾取成功：背包中潜影盒数量增加了
        if (countShulkersInInventory() > pickupBefore) {
            baritone.getPathingBehavior().cancelEverything();
            resetPickupState();
            return 1; // DONE
        }
        ...
        // 超时放弃（~10 秒）
        if (++pickupAttempts > PICKUP_TIMEOUT_TICKS) {
            baritone.getPathingBehavior().cancelEverything();
            resetPickupState();
            return 2; // GAVE_UP
        }

        // （重新）获取掉落的潜影盒实体 —— 打碎后可能弹开一格
        if (pickupDrop == null || !pickupDrop.isAlive() || pickupDrop.isRemoved()) {
            pickupDrop = nearestShulkerDrop(breakPos);
            pickupPathed = false;
            pickupSettleTicks = 0;
            pickupGoal = null;
        }
        ...
        if (pickupDrop != null) {
            // 落地等待：新打碎的潜影盒会弹跳，等它落地再追（避免追到过时位置）
            if (!pickupDrop.isOnGround() && ++pickupSettleTicks < PICKUP_SETTLE_TICKS) {
                return 0; // BUSY — 还在等落地
            }
            // 漂移追踪：掉落物弹到新方块时重新寻路
            ...
            if (!pickupPathed || !cur.equals(pickupGoal)) {
                ...
                pickupGoal = cur;
                pickupPathed = true;
                if (logActions.get()) dbg("追逐潜影盒掉落物 → %s", cur.toShortString());
            }
     ...
     * 在破坏点附近搜索掉落的潜影盒实体。
     */
    private ItemEntity nearestShulkerDrop(BlockPos near) {
        ...
        for (ItemEntity e : mc.world.getEntitiesByClass(ItemEntity.class, new Box(near).expand(4),
            e -> e.isAlive() && isShulkerBoxItem(e.getStack()))) {
    ...
    /**
     * 统计背包中潜影盒的总数量（物品数量，非槽位数）。
     */
    private int countShulkersInInventory() {
        int n = 0;
        for (int i = 0; i < 36; i++) {
            ItemStack s = mc.player.getInventory().getStack(i);
            if (isShulkerBoxItem(s)) n += s.getCount();
        }
        return n;
    }

    /**
     * 使用 Baritone 寻路到指定方块（精确站在上面），用于拾取掉落物。
     */
    private void pathToBlock(BlockPos pos) {
...
//        mc.player.setPitch(pitch);
    ...
    private void dbg(String fmt, Object... args) {
        if (debugMode.get()) info(args.length == 0 ? fmt : String.format(fmt, args));
    }
}
